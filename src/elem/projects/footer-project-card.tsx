import "./footer-project-card.css"
        ...
        .then((assetsID: string) => {
            if (assetsID != null) {
                return octokit.request(
                    "GET /repos/" + project.githubIdentifier + "/releases/{assetsURL}/assets",
                    {assetsURL: assetsID})
            } else {
                return new Promise<OctokitResponse<any>>(() => null);
            }
        })
        ...
        return (<div className="buttons">
                {files.map((asset: { name: string; browser_download_url: string; }) => {
                    if (asset.name.endsWith(".exe")) {
                        return (<RoundButton leftGraphic={<FaWindows/>} text="Windows"
                                             link={asset.browser_download_url}/>)
                    } else if (asset.name.endsWith(".zip")) {
                        return (<RoundButton leftGraphic={<FaFileArchive/>} text="Portable"
                                             link={asset.browser_download_url}/>)
                    } else if (asset.name.endsWith(".deb")) {
                        return (<RoundButton leftGraphic={<FaDebian/>} text="Debian"
                                             link={asset.browser_download_url}/>)
                    } else if (asset.name.endsWith(".dmg")) {
                        return (<RoundButton leftGraphic={<FaApple/>} text="Debian"
                                             link={asset.browser_download_url}/>)
                    }
                })}
            </div>)
    } else {
        return (<div>No file currently available for this project</div>)
    }
}
...
export default function FooterProjectCard({project}: { project: Project }) {
    return (<div id="header-card" className="footer-project-card">
